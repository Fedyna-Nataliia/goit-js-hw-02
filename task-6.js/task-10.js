...
const users =  [
  ...
  const getSortedUniqueSkills = (array) => array.reduce((acc, user) => acc.concat(...user.skills), []).filter((skill, index, arr) => arr.indexOf(skill) === index).sort();
// не працює
  //const getSortedUniqueSkills = (array) => array.reduce((acc, user) => [...acc, ...skills], []).filter((skill, index, arr) => arr.indexOf(skill) === index).sort();